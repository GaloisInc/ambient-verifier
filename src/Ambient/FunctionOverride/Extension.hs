...
import qualified ODSL.C.Syntax as ODSLC ( newGCC, parseCFile )
import qualified ODSL.C.Transform as ODSLC ( simpl )
import qualified ODSL.Crucible.Transform as ODSLCrucible ( simpl )
import qualified ODSL.Translate as ODSLT ( translate )
...
loadCOverride
  ...
  -- ^ ParserHooks for the desired syntax extension
  -> IO (LCSC.ParsedProgram ext)
loadCOverride path cc ng halloc hooks =
  ...
  IO.withFile (odslIncludesDir SF.</> AODSLP.odslCrucibleHeaderPath) IO.WriteMode $ \crucibleHeaderHdl -> do
    ...
    -- ...next, parse the C file...
    res <- ODSLC.parseCFile (ODSLC.newGCC cc) Nothing ["-I" ++ odslIncludesDir] path
    -- ...then compile it to crucible-syntax...
    transUnit <- case res of
      Left parseErr   -> CMC.throwM $ AE.COverrideParseError path parseErr
      Right transUnit -> pure transUnit
    simplTransUnit <- ODSLC.simpl transUnit
    prog <- case ODSLT.translate simplTransUnit of
      Left transErr -> CMC.throwM $ AE.COverrideTransError path transErr
      Right prog    -> pure prog
    simplProg <- ODSLCrucible.simpl prog
    ...
    let contents = prettyPrint simplProg
...
runOverrideTests logAction bak archInfo archVals (AF.BuildFunctionABI buildFunctionABI)
  ...
  cblProgs <-
    traverse (\path -> (path,) <$> loadCblOverride path ng halloc hooks)
             cblFiles
  cProgs <-
    traverse (\path -> (path,) <$> loadCOverride path cc ng halloc hooks)
             cFiles
