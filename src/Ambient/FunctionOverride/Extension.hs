...
loadCOverride
  ...
  -- ^ ParserHooks for the desired syntax extension
  -> IO (LCSC.ParsedProgram ext)
loadCOverride _archInfo path cc ng halloc hooks =
  ...
  IO.withFile (odslIncludesDir SF.</> AODSLP.odslCrucibleHeaderPath) IO.WriteMode $ \crucibleHeaderHdl -> do
    ...
    res <- runExceptT $ ODSLC.translateFile (odslImpl $ natVal (Proxy @w)) odslIncludesDir path
    prog <- case res of
  ...
  where
    odslImpl :: Natural -> ODSLC.Impl
    odslImpl 32 = ODSLC.implAArch32
    odslImpl _  = ODSLC.implX64
...
runOverrideTests logAction bak archInfo archVals (AF.BuildFunctionABI buildFunctionABI)
  ...
  cblProgs <-
    traverse (\path -> (path,) <$> loadCblOverride path ng halloc hooks)
             cblFiles
  cProgs <-
    traverse (\path -> (path,) <$> loadCOverride archInfo path cc ng halloc hooks)
             cFiles
...
loadCrucibleSyntaxOverrides archInfo dirPath cc ng halloc hooks = do
