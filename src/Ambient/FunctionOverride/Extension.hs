...
import qualified ODSL.C.Syntax as ODSL ( newGCC, parseCFile )
import qualified ODSL.C.Transform as ODSL ( simpl )
import qualified ODSL.Translate as ODSL ( translate )
...
loadCOverride
  ...
  -- ^ ParserHooks for the desired syntax extension
  -> IO (LCSC.ParsedProgram ext)
loadCOverride path cc ng halloc hooks = do
  -- First, parse the C file...
  res <- ODSL.parseCFile (ODSL.newGCC cc) Nothing [] path
  ...
  prog <- case ODSL.translate (ODSL.simpl transUnit) of
