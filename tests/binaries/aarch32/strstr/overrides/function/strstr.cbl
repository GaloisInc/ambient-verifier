(defun @strstr ((haystack Pointer) (needle Pointer)) Pointer
  ...
  (defblock loop:
    ...
    (let haystack-front (pointer-read 1 le haystacki))
    (branch (not (equal? haystack-front (bv 8 0))) loop-body-1: exit-null:))

  (defblock loop-body-1:
    (let needle-front (pointer-read 1 le needle))
...
(defun @compare ((x Pointer) (y Pointer)) Int
  ...
  (defblock loop:
    ...
    (let xi-front (pointer-read 1 le xi))
    (let yi-front (pointer-read 1 le yi))
    (let xi-nezero (not (equal? xi-front (bv 8 0))))
    (let yi-nezero (not (equal? yi-front (bv 8 0))))
